module LibraryItemsHelper
  ...
  def render_request_library_item_link(library_item)
	  link_to "Request Item", "http://#{ catalogue_opac_addr }/search?/.#{ library_item.bib_no.chop.to_s }/.#{ library_item.bib_no.chop.to_s }/1%2C1%2C1%2CB/request~".html_safe, {:class => "btn btn-small square-corners"}
  end	

  def render_bookable_library_item_link(library_item)
    link_to "Book Item", "http://#{ catalogue_opac_addr }/webbook?/#{ library_item.bib_no.chop.to_s }&back=/search".html_safe, {:class => "btn btn-small square-corners"}
  end 
